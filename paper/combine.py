...
# Load an image
image = cv2.imread('D:/project on filter/paper/113.jpg')
...
cv2.imwrite('thekmean.jpg', segmented_image )




threshold_value = 90
mask = np.where(segmented_image > threshold_value,  255, 0).astype(np.uint8)

contrast = 1.5  # was 1.5
brightness = 16  # was 10
adjusted_image = cv2.convertScaleAbs(segmented_image , alpha=contrast, beta=brightness)


# adjusted_image = cv2.convertScaleAbs(mask, alpha=contrast, beta=brightness)

blurred_image11 = cv2.GaussianBlur(mask, (5,5), 0)
sharpened_image11 = cv2.addWeighted(mask, 3.0, blurred_image11, -0.7,0 )
...
cv2.imwrite('113a.jpg', sharpened_image111)
...
# ground_truth = io.imread('ground_truth.png')
# segmented_image = io.imread('segmented_image.png')
...
# # Flatten images to 1D arrays for computing metrics
# ground_truth_flat = ground_truth.flatten()
# segmented_flat = segmented_image.flatten()

# # Threshold the images to create binary masks
# threshold = 0.5  # Adjust as needed based on image characteristics
# ground_truth_binary = (ground_truth_flat > threshold).astype(np.uint8)
# segmented_binary = (segmented_flat > threshold).astype(np.uint8)
